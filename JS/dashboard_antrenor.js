...
// Functie pentru afisarea antrenorilor
function afiseazaAntrenori() {
  const container = document.getElementById('antrenoriContainer');
  container.innerHTML = '';

  let antrenori = JSON.parse(localStorage.getItem('users')) || [];
  antrenori = antrenori.filter(user => user.role === 'Antrenor');

  if(antrenori.length === 0) {
    container.innerHTML = '<p style="color: white;">Nu există antrenori disponibili momentan.</p>';
    return;
  }
  ...
  antrenori.forEach(antrenor => {
    ...
    card.innerHTML = `
      <img src="IMAGES/default_profile.png" alt="Profil Antrenor">
      <h3>${antrenor.username}</h3>
      <p>${antrenor.email}</p>
      <button class="btn-solicita" onclick="openAntrenorModal('${antrenor.username}', '${antrenor.email}')">Detalii</button>
    `;
...
// Functii pentru Modal
function openAntrenorModal(nume, email) {
  document.getElementById('antrenorNume').textContent = nume;
  document.getElementById('antrenorEmail').textContent = email;
  document.getElementById('antrenorImg').src = "IMAGES/default_profile.png";
...
// Functia pentru inchiderea sectiunii cu smooth scroll
function inchideAntrenori() {
  const sectiuneAntrenori = document.getElementById('select-antrenor');
  sectiuneAntrenori.style.display = 'none';

  // Te duce inapoi catre sectiunea Servicii smooth
